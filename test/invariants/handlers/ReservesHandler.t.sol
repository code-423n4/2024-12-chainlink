...
contract ReservesHandler is Test, Constants {
    ...
    function setEarmarks(uint256 serviceProviderIndex, int96 amount) public {
        ...
        int96 earmarkAmount = int96(
            bound(amount, (-1 * LINK_MAX_SUPPLY) - s_totalEarmarked, LINK_MAX_SUPPLY - s_totalEarmarked)
        );
    ...
    function withdraw(uint256 serviceProviderIndex) public {
        address serviceProvider = _getServiceProvider(serviceProviderIndex);
    ...
    function _getServiceProvider(uint256 index) private view returns (address) {
