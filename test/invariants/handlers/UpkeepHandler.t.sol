...
contract UpkeepHandler is BaseTest {
    ...
    function performUpkeep(UpkeepParams[3] memory params, uint256 linkPrice) public {
        ...
        for (uint256 i; i < params.length; ++i) {
            ...
            uint256 amountOutFromOracle = _convertToLink(
                swapAmount,
                IERC20Metadata(asset),
                AggregatorV3Interface(swapParams.oracle)
            );
            uint256 amountOutOracleWithSlippage = amountOutFromOracle.percentMul(
            ...
            uint256 minPostSwapAmount = amountOutFromOracle.percentMul(
                PercentageMath.PERCENTAGE_FACTOR - swapParams.maxPriceDeviation
            );
    ...
    function _convertToLink(
        ...
        if (tokenDecimals < 18) {
            return
                (assetAmount * _getAssetPrice(oracle) * 10 ** (18 - tokenDecimals)) / _getAssetPrice(s_mockLinkUsdFeed);
        } else {
            return
                (assetAmount * _getAssetPrice(oracle)) / _getAssetPrice(s_mockLinkUsdFeed) / 10 ** (tokenDecimals - 18);
        }
    }
