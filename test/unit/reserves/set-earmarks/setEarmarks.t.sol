...
contract Reserves_SetEarmarksUnitTest is BaseUnitTest {
    ...
    function test_setEarmarks_SingleEarmarkAmountGtZero() public {
        ...
        Reserves.ServiceProvider memory expectedServiceProvider = Reserves.ServiceProvider(
            s_singleEarmark[0].amountLinkOwed,
            1
        );
    ...
    function test_setEarmarks_MultipleEarmarksAmountGtZero() public {
        ...
        Reserves.ServiceProvider memory expectedServiceProvider1 = Reserves.ServiceProvider(
            s_multipleEarmarks[0].amountLinkOwed,
            1
        );
        Reserves.ServiceProvider memory expectedServiceProvider2 = Reserves.ServiceProvider(
            s_multipleEarmarks[1].amountLinkOwed,
            1
        );
    ...
    function test_setEarmarks_RevertWhen_CallerDoesNotHaveTheEARMARK_MANAGER_ROLE()
        public
        whenCallerIsNotEarmarkManager
    {
        vm.expectRevert(
            abi.encodeWithSelector(
                IAccessControl.AccessControlUnauthorizedAccount.selector,
                OWNER,
                Roles.EARMARK_MANAGER_ROLE
            )
        );
        s_reserves.setEarmarks(s_multipleEarmarks);
    }
