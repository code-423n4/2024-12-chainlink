...
contract CheckUpkeepUnitTest is BaseUnitTest {
    ...
    modifier whenPaused(address contractAddress) {
        _changePrank(PAUSER);
        PausableWithAccessControl(contractAddress).emergencyPause();
        _;
    }

    modifier givenOracleDataIsZero(address oracle) {
    ...
    modifier givenOracleDataIsStale(address oracle) {
    ...
    modifier givenAssetSufficientBalanceForSwap(address asset) {
        SwapAutomator.SwapParams memory swapParams = s_swapAutomator.getAssetSwapParams(asset);
        uint256 assetBalance = (swapParams.maxSwapSizeUsd * 10 ** IERC20Metadata(asset).decimals()) /
            _getAssetPrice(swapParams.oracle);
    ...
    modifier givenAssetInsufficientBalanceForSwap(address asset) {
        vm.mockCall(
            asset,
            abi.encodeWithSelector(IERC20.balanceOf.selector, address(s_feeAggregatorReceiver)),
            abi.encode(0)
    ...
    modifier givenAssetElapsedTimeSinceLastSwapLtSwapInterval(address asset) {
    ...
    function setUp() public {
        ...
        s_swapAutomator.applyAssetSwapParamsUpdates(
            new address[](0),
            SwapAutomator.AssetSwapParamsArgs({assets: assets, assetsSwapParams: swapParams})
        );
