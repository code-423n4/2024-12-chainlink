...
contract AddAllowlistedSendersUnitTest is BaseUnitTest {
    ...
    function setUp() public {
        FeeAggregator.AllowlistedSenders memory allowlistedSenders = FeeAggregator.AllowlistedSenders({
            sourceChainSelector: SOURCE_CHAIN_1,
            senders: new bytes[](2)
        });
    ...
    function test_addAllowlistedSenders_RevertWhen_SenderIsAlreadyAllowlisted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.SenderAlreadyAllowlisted.selector,
                SOURCE_CHAIN_1,
                s_allowlistedSenders[0].senders[0]
            )
        );
        s_feeAggregatorReceiver.applyAllowlistedSenders(s_emptySenders, s_allowlistedSenders);
    }
    ...
    function test_addAllowlistedSenders_SingleSender() external {
        ...
        FeeAggregator.AllowlistedSenders memory newAllowlistedSenders = FeeAggregator.AllowlistedSenders({
            sourceChainSelector: SOURCE_CHAIN_1,
            senders: new bytes[](1)
        });
    ...
    function test_addAllowlistedSender_SingleSenderAlreadyAllowlistedSourceChain() external {
        ...
        delete s_allowlistedSenders;
        FeeAggregator.AllowlistedSenders memory newAllowlistedSenders2 = FeeAggregator.AllowlistedSenders({
            sourceChainSelector: SOURCE_CHAIN_1,
            senders: new bytes[](1)
        });
