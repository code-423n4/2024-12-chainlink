...
contract AddAllowlistedReceiversUnitTest is BaseUnitTest {
    ...
    function test_addAllowlistedReceivers_RevertWhen_CallerDoesNotHaveDEFAULT_ADMIN_ROLE() public whenCallerIsNotAdmin {
        ...
        FeeAggregator.AllowlistedReceivers memory allowlistedReceiver = FeeAggregator.AllowlistedReceivers({
            destChainSelector: SOURCE_CHAIN_1,
            receivers: new bytes[](2)
        });
    ...
    function test_addAllowlistedReceivers_RevertWhen_ReceiverIsAlreadyAllowlisted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.ReceiverAlreadyAllowlisted.selector,
                DESTINATION_CHAIN_1,
                newReceivers[0].receivers[0]
            )
        );
        s_feeAggregatorReceiver.applyAllowlistedReceivers(emptyReceivers, newReceivers);
    }
    ...
    function test_addAllowlistedReceivers_SingleReceiver() external {
        ...
        FeeAggregator.AllowlistedReceivers memory newAllowlistedReceiver = FeeAggregator.AllowlistedReceivers({
            destChainSelector: DESTINATION_CHAIN_1,
            receivers: new bytes[](1)
        });
    ...
    function test_addAllowlistedReceiver_ReceiverAlreadyAllowlistedDestinationChain() external {
        ...
        FeeAggregator.AllowlistedReceivers memory newAllowlistedReceiver2 = FeeAggregator.AllowlistedReceivers({
            destChainSelector: DESTINATION_CHAIN_1,
            receivers: new bytes[](1)
        });
    ...
    function test_addAndRemoveAllowlistedReceiver() public {
        ...
        receiversToAdd[0] = FeeAggregator.AllowlistedReceivers({
            destChainSelector: DESTINATION_CHAIN_1,
            receivers: new bytes[](1)
        });
        ...
        receiversToRemove[0] = FeeAggregator.AllowlistedReceivers({
            destChainSelector: DESTINATION_CHAIN_1,
            receivers: new bytes[](1)
        });
        ...
        assertEq(
            allowlistedDestinationChains[0],
            DESTINATION_CHAIN_1,
            "Destination chain should be DESTINATION_CHAIN_1"
        );
    }
}
