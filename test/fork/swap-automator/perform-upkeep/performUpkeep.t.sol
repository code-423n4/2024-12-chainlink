...
contract PerformUpkeepForkTest is BaseForkTest {
    ...
    modifier givenAssetEligibleForSwap(address asset) {
    ...
    function test_performUpkeep_RevertWhen_TxTimestampGtDeadline() public givenAssetEligibleForSwap(WETH) {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Errors.TransactionTooOld.selector, block.timestamp, block.timestamp - 1)
        );
        ...
        givenAssetEligibleForSwap(USDC)
    {
        ...
        (IV3SwapRouter.ExactInputParams[] memory swapInputs, ) = abi.decode(
            data,
            (IV3SwapRouter.ExactInputParams[], uint256)
        );
        ...
        givenAssetEligibleForSwap(USDC)
    {
        ...
        (IV3SwapRouter.ExactInputParams[] memory swapInputs, uint256 deadline) = abi.decode(
            data,
            (IV3SwapRouter.ExactInputParams[], uint256)
        );
    ...
    function test_performUpkeep_RevertWhen_SingleAssetSwapPathChangedToEmpty() public givenAssetEligibleForSwap(WETH) {
        ...
        bytes memory unmatchedPath = bytes.concat(
            bytes20(WETH),
            bytes3(uint24(3000)),
            bytes20(WETH),
            bytes3(uint24(3000)),
        ...
        givenAssetEligibleForSwap(LINK)
    {
        ...
        vm.mockCall(
            LINK,
            abi.encodeWithSelector(LinkTokenInterface.transfer.selector, RECEIVER, amountInLink),
            abi.encode(false)
        );
        ...
        givenAssetEligibleForSwap(USDC)
    {
        ...
        uint256 linkBalanceValue = _getAssetPrice(AggregatorV3Interface(LINK_USD_FEED)) *
            IERC20(LINK).balanceOf(RECEIVER);
        ...
        givenAssetEligibleForSwap(WETH)
    {
        ...
        uint256 amountOutFromOracle = _getExpectedAmountOutLinkFromOracle(
            amountIn,
            abtAssetPrice,
            IERC20Metadata(WETH)
        );
        ...
        // threshold.
        uint256 mockAmountBelowDeviationThreshold = amountOutFromOracle.percentMul(
            PercentageMath.PERCENTAGE_FACTOR - MAX_PRICE_DEVIATION * 2
        );
        vm.mockCall(
            UNISWAP_ROUTER,
            abi.encodeWithSelector(IV3SwapRouter.exactInput.selector),
        ...
        givenAssetEligibleForSwap(USDC)
    {
        ...
        uint256 amountOutWethFromOracle = _getExpectedAmountOutLinkFromOracle(
            amountInWeth,
            wethAssetPrice,
            IERC20Metadata(WETH)
        );
        ...
        uint256 mockAmountOutWETHBelowDeviationThreshold = amountOutWethFromOracle.percentMul(
            PercentageMath.PERCENTAGE_FACTOR - MAX_PRICE_DEVIATION * 2
        );
        vm.mockCall(
            UNISWAP_ROUTER,
    ...
    function test_performUpkeep_SingleSwapSinglePool() public givenAssetEligibleForSwap(WETH) {
        ...
        (uint256 amountOutUniswapQuote, , , ) = s_swapAutomator.getUniswapQuoterV2().quoteExactInput(
            swapParams.path,
            amountIn
        );
        uint256 amountOutFromOracle = _getExpectedAmountOutLinkFromOracle(
        ...
        uint256 minAmountOutValue = _getAssetPrice(AggregatorV3Interface(LINK_USD_FEED)) *
            Math.max(amountOutFromOracle, amountOutUniswapQuote).percentMul(
                PercentageMath.PERCENTAGE_FACTOR - MAX_SLIPPAGE
            );
    ...
    function test_performUpkeep_MultipleSwaps() public givenAssetEligibleForSwap(WETH) givenAssetEligibleForSwap(USDC) {
        ...
        SwapAutomator.SwapParams memory swapParams = s_swapAutomator.getAssetSwapParams(WETH);
        (uint256 amountOutWethUniswapQuote, , , ) = s_swapAutomator.getUniswapQuoterV2().quoteExactInput(
            swapParams.path,
            amountInWeth
        );
        uint256 amountOutWethFromOracle = _getExpectedAmountOutLinkFromOracle(
        ...
        (uint256 amountOutUsdcUniswapQuote, , , ) = s_swapAutomator.getUniswapQuoterV2().quoteExactInput(
            swapParams.path,
            amountInUsdc
        );
        uint256 amountOutUsdcFromOracle = _getExpectedAmountOutLinkFromOracle(
            amountInUsdc,
            usdcAssetPrice,
            IERC20Metadata(USDC)
        );
        ...
        uint256 linkBalanceValue = _getAssetPrice(AggregatorV3Interface(LINK_USD_FEED)) *
            IERC20(LINK).balanceOf(address(RECEIVER));
    ...
    function test_performUpkeep_SwapAllAbts() public {
        ...
        for (uint256 i = 0; i < allowlistedAssets.length; ++i) {
            ...
            if (allowlistedAssets[i] != LINK) {
                ...
                (amountOutUniswapQuote, , , ) = s_swapAutomator.getUniswapQuoterV2().quoteExactInput(
                    swapParams.path,
                    amountIn
                );
            }
            uint256 amountOutFromOracle = _getExpectedAmountOutLinkFromOracle(
                amountIn,
                assetPrice,
                IERC20Metadata(allowlistedAssets[i])
            );
            totalMinAmountOut += Math.max(amountOutFromOracle, amountOutUniswapQuote).percentMul(
                PercentageMath.PERCENTAGE_FACTOR - MAX_SLIPPAGE
            );
        }
