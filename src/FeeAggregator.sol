...
contract FeeAggregator is IFeeAggregator, EmergencyWithdrawer, ITypeAndVersion, ILinkAvailable, CCIPReceiver {
    ...
    /// @inheritdoc IFeeAggregator
    function areAssetsAllowlisted(address[] calldata assets) external view returns (bool, address) {
        for (uint256 i; i < assets.length; ++i) {
            if (!s_allowlistedAssets.contains(assets[i])) {
    ...
    function getAllowlistedSenders(uint64 chainSelector) external view returns (bytes[] memory) {
    ...
    function bridgeAssets(
        ...
        Client.EVM2AnyMessage memory evm2AnyMessage = _buildBridgeAssetsMessage(
            bridgeAssetAmounts,
            bridgeReceiver,
            data,
            extraArgs
    ...
    function getAllowlistedReceivers(uint64 destChainSelector) external view returns (bytes[] memory) {
    ...
    function supportsInterface(
        ...
        return
            ...
            interfaceId == type(IFeeAggregator).interfaceId ||
