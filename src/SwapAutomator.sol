...
contract SwapAutomator is ITypeAndVersion, PausableWithAccessControl, AutomationCompatible {
    ...
    constructor(
        ...
        if (
            params.linkToken == address(0) ||
            params.linkUsdFeed == address(0) ||
    ...
    function applyAssetSwapParamsUpdates(
        ...
        (bool areAssetsAllowlisted, address nonAllowlistedAsset) = s_feeAggregator.areAssetsAllowlisted(
            assetSwapParamsArgs.assets
        );
        ...
        for (uint256 i; i < assetSwapParamsArgs.assetsSwapParams.length; ++i) {
            ...
            s_assetHashedSwapPath[assetSwapParamsArgs.assets[i]] = keccak256(
                assetSwapParamsArgs.assetsSwapParams[i].path
            );
    ...
    function getAssetSwapParams(address asset) external view returns (SwapParams memory) {
    ...
    function setFeeAggregator(address feeAggregator) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setDeadlineDelay(uint96 deadlineDelay) external onlyRole(Roles.ASSET_ADMIN_ROLE) {
        _setDeadlineDelay(deadlineDelay);
    }

    /// @notice Sets the deadline delay
    /// @param deadlineDelay The new deadline delay
    function _setDeadlineDelay(uint96 deadlineDelay) internal {
    ...
    /// @param asset The address of the asset
    function getLatestSwapTimestamp(address asset) external view returns (uint256) {
    ...
    function getHashedSwapPath(address asset) external view returns (bytes32) {
    ...
    function setLinkReceiver(address linkReceiver) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /// @param linkReceiver The address of the address that will
    /// receive swapped LINK
    function _setLinkReceiver(address linkReceiver) internal {
    ...
    function checkUpkeep(bytes calldata) external whenNotPaused cannotExecute returns (bool, bytes memory) {
        address[] memory allowlistedAssets = s_feeAggregator.getAllowlistedAssets();
        IV3SwapRouter.ExactInputParams[] memory swapInputs = new IV3SwapRouter.ExactInputParams[](
            allowlistedAssets.length
        );
        ...
        for (uint256 i; i < allowlistedAssets.length; ++i) {
            ...
            if (
                ...
                uint256 swapAmountIn = Math.min(swapParams.maxSwapSizeUsd * assetUnit, availableAssetUsdValue) /
                    assetPrice;

                // 4) Quote the amountOut from both Uniswap V3 quoter and CL oracle price feed for all ADTs
                // except LINK
                uint256 amountOutUniswapQuote;
                uint256 amountOutCLPriceFeedQuote = _convertToLink(
                    swapAmountIn,
                    assetPrice,
                    linkUSDPrice,
    ...
    function performUpkeep(bytes calldata performData) external whenNotPaused {
        ...
        for (uint256 i; i < swapInputs.length; ++i) {
            ...
            if (asset == address(i_LINK)) {
                ...
                try this.swapWithPriceFeedValidation(swapInputs[i], asset, linkPriceFromFeed) returns (
                    uint256 amountOut
                ) {
    ...
    function _getAssetPrice(AggregatorV3Interface oracle) private view returns (uint256, uint256) {
